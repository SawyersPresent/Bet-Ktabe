...
[PATH](http://www.linfo.org/path_env_var.html) allows the user to type a command and use a tool without needing to specifying its absolute path. we can check this current path easily 
...
// then we modify to add the working directory
PATH=.:${PATH}
export PATH
echo $PATH

// now we can modify and create a binary that does whatever we want that we can call from whereever. the binary must stay where it is tho
touch wtf
echo 'echo "what the fuck nigga"' > wtf
chmod +x wtf
