...
https://github.com/0xHillside/BackupPrivilege-Script.git
...
If the privilege is disabled, we can enable it with `Set-SeBackupPrivilege`. Note: Based on the server's settings, it might be required to spawn an elevated CMD prompt to bypass UAC and have this privilege.
...
Incase there is a protected file what we can do is copy it 
...
PoC for abusing BackUpOperators group privilege

https://github.com/giuliano108/SeBackupPrivilege


another way to exploit this group would be using secretsdump 



```python
Hillside_1@htb[/htb]$ secretsdump.py -ntds ntds.dit -system SYSTEM -hashes lmhash:nthash LOCAL
