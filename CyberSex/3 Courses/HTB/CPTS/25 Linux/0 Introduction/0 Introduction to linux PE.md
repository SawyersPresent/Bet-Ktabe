...
The root account on a linux system essentially provides **full** administrator level access to the operating system, getting to this account means fully compromising the host.


# Enumeration

Enumeration is the essential things that needed for privesc, several helper scripts like linenum exists to assist with enumeration. still its important to look for this manually


- OS Version
	- knowing the distribution helps know what tools are available
	- help identify which vulnerabilities work and which dont
- Kernal Version
	- Kernel exploits can cause instability and crashes
	- make sure to fully understand the exploit before running it
	- public exploits for kernels exist
- Running Services
	- knowing what services are running are important especially the services running as root
	- a misconfigured service running as root can be easy privesc
	- list common services can be done with listing processes that are running
		- `ps aux | grep root`
- Installed Packages & versions
- logged in users
	- through listed processes
...
```shell-session
Hillside_1@htb[/htb]$ ps aux | grep root
...
```shell-session
Hillside_1@htb[/htb]$ ls -la /home/stacey.jenkins/
...
```shell-session
Hillside_1@htb[/htb]$ cat /etc/passwd
...
```shell-session
Hillside_1@htb[/htb]$ ls -la /etc/cron.daily/
...
```shell-session
Hillside_1@htb[/htb]$ find / -path /proc -prune -o -type d -perm -o+w 2>/dev/null
