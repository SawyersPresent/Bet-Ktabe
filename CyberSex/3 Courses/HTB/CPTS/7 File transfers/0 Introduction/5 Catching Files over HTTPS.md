...
```shell-session
Hillside_1@htb[/htb]$ sudo mkdir -p /var/www/uploads/SecretUploadDirectory
```
...
```shell-session
Hillside_1@htb[/htb]$ sudo chown -R www-data:www-data /var/www/uploads/SecretUploadDirectory
```
...
Hillside_1@htb[/htb]$ sudo ln -s /etc/nginx/sites-available/upload.conf /etc/nginx/sites-enabled/
```
...
```shell-session
Hillside_1@htb[/htb]$ sudo systemctl restart nginx.service
```
...
```shell-session
Hillside_1@htb[/htb]$ tail -2 /var/log/nginx/error.log
...
```shell-session
Hillside_1@htb[/htb]$ ss -lnpt | grep 80
...
```shell-session
Hillside_1@htb[/htb]$ ps -ef | grep 2811
...
```shell-session
Hillside_1@htb[/htb]$ sudo rm /etc/nginx/sites-enabled/default
```
...
```shell-session
Hillside_1@htb[/htb]$ curl -T /etc/passwd http://localhost:9001/SecretUploadDirectory/users.txt
```

  Catching Files over HTTP/S

```shell-session
Hillside_1@htb[/htb]$ sudo tail -1 /var/www/uploads/SecretUploadDirectory/users.txt 
