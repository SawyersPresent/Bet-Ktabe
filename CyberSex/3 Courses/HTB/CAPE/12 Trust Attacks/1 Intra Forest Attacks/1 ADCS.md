...
ADCS stores details regarding `Certification Authorities (CAs)` and `Certificate Templates` within `Active Directory Domain Services (AD DS)`, which is a core component of Microsoft’s Active Directory. Access to this information is provided through the `Lightweight Directory Access Protocol (LDAP)`, which enables applications and services to query and interact with directory data. By using AD DS as the storage service and LDAP as the access protocol, ADCS integrates seamlessly with Active Directory, allowing administrators to manage certificate services using familiar tools and interfaces within the Active Directory environment.
...
![ADSI Edit window showing CN=INLANEFREIGHT-DC01-CA under CN=Enrollment Services. The properties window is open to the "Attribute Editor" tab, displaying attributes like cACertificateDN and certificateTemplates with values such as "DirectoryEmailReplication."](https://cdn.services-k8s.prod.aws.htb.systems/content/modules/253/dc02_edit_pkienrollment.png)
...
- **Correctly identified an ADCS misconfiguration** that allows certificate enrollment with arbitrary SANs.
- **Successfully duplicated and used a vulnerable certificate template** (`Copy of User`).
- **Issued certificates with alternate identities (SAN UPN abuse)**, which is the core ADCS attack primitive.
- **Understood and used PKINIT correctly** to authenticate with certificates instead of passwords.
- **Successfully obtained Kerberos TGTs and TGSs** using both Rubeus and Certipy.
- **Verified ticket injection and presence** using `klist`, confirming Kerberos was functioning.
- **Correctly targeted the parent domain CA from the child context**, aligning with a trust-abuse scenario.
- **Persisted through tooling differences (Windows vs Linux)** and adapted workflows between them.


### what I did wrong

- **Authenticated with certificates whose SAN identity did not match the `/user:` you intended**, causing identity mismatch.
- **Reused Administrator-issued certificates when the lab required Tom’s identity**, breaking authorization.
- **Failed to purge existing Kerberos tickets before switching identities**, causing Windows to reuse old tickets
- **Assumed successful TGS issuance implied file access**, ignoring share and NTFS ACL enforcement.
- **Repeatedly accessed administrative shares (`C$`) instead of the intended custom share**, guaranteeing access denial.
- **Mixed domain formats (`.local` vs `.ad`, UPN vs down-level names)**, causing Kerberos validation failures
- **Stacked delegated and forwarded tickets unintentionally**, leading to confusing ticket selection behavior
- **Focused on “getting more tickets” instead of validating which identity was actually being used** at access time.
